...
class DashboardHR extends Controller
{
    public function searchDashboard(Request $request)
    {
        ...
        if($searchall != ""){
            ...
            foreach ($department as $key => $departments) {
                $user = User::where('role', '!=', 'moderator')->where('role', '!=', 'admin')->where('department', $departments->name)->where('name', 'like', '%'.$searchall.'%')->orderByRaw($unitOrder)->get();
                array_push($departmentArr, $user);
                // $user->appends(['all' => $searchall]);
            }
