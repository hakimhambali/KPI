...
{{--------------------------------------------------- KPI MASTER DISPLAY --------------------------------------------------}}
                ...
                @foreach ($kpiArrs as $key4 => $kpiArrss)
                  ...
                  @if($kpiArrss->fungsi == $functions->name)
                    <tr>
                      @if ($key4 == 0)
                        <td rowspan="{{ $kpiArrs->count() }}">
                          <p class="text-xs font-weight-bold mb-0 px-3" value="{{ $kpiArrss->fungsi }}">{{ $kpiArrss->fungsi }}</p>
                        </td>
                        <td rowspan="{{ $kpiArrs->count() }}" class="text-xs font-weight-bold mb-0">
                          <span class="text-secondary text-xs font-weight-bold" value="{{ $kpiArrss->kpimasters->objektif }}">{{ $kpiArrss->kpimasters->objektif }}</span>
                        </td>
                      @else
                      @endif
                      @if ($loop->parent->last)
                        <td class="text-xs font-weight-bold">
                          <span class="text-secondary text-xs font-weight-bold" value="{{ $kpiArrss->bukti }}">{{ $kpiArrss->bukti }}</span>
                        </td>
                        <td class="text-xs font-weight-bold">
                          @if ($kpiArrss->bukti_path == '')
                          <a href=" {{ $kpiArrss->bukti_path }}" style="color:blue;text-decoration:underline;font-size:13.5px"; target="_blank"></a>
                          @else
                          <a href=" {{ $kpiArrss->bukti_path }}" style="color:blue;text-decoration:underline;font-size:13.5px"; target="_blank">View</a>
                          @endif
                        </td>
                      @else
